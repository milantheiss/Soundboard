<template>
	<div>
		<button @click="playSoundeffect" class="border border-black m-6" :class="this.effect.player.playing() && !this.isStopping ? 'bg-gray-300' : ''">
			{{ effect.name }}
		</button>
	</div>
</template>
	...
	methods: {
		...
		startEffect() {
			...
			if (this.effect.player.playing()) {
				...
				//Wenn Effekt nicht spielt, wird er normal von Vol 0 gestartet.
				this.effect.player.play();
				this.effect.player.fade(0.0, this.effect.trackvolume, this.effect.fadeInDuration);
				this.isStarting = true;
			}
